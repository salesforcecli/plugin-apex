...
describe('force:apex:log:list', () => {
  ...
  test
    ...
    .stdout()
    .command(['force:apex:log:list', '-o', 'test@username.com'])
    .it('runs default command with cleaned table output', (ctx) => {
      ...
      expect(ctx.stdout).to.include(
        ' Unknown     75            07L5tgg0005PGdTnEAL Unknown  450      Test User  API       API     2020-10-13T05:39:43+0000 Assertion Failed'
      );
      expect(ctx.stdout).to.include(
        'Unknown     75            07L5tgg0005PGdTnFPL Unknown  450      Test User2 API       API     2020-10-13T05:39:43+0000 Successful'
      );
    });
  ...
  test
    ...
    .stub(LogService.prototype, 'getLogRecords', () => logRecords)
    .stdout()
    .command(['force:apex:log:list', '-o', 'test@username.com', '--json'])
  ...
  test
    ...
    .it('should return json output if no logs were found', (ctx) => {
      const emptyResult = JSON.stringify({ status: 0, result: [], warnings: [] }, null, 2);
